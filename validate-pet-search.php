...
if (
    ...
    $_SESSION['email'] && (!empty(htmlspecialchars($_GET['breed'], ENT_QUOTES))
        ||
    ...
    $filtered_pets_by_breed = array();
    $breed = htmlspecialchars($_GET['breed'], ENT_QUOTES);
    $gender = htmlspecialchars($_GET['gender'], ENT_QUOTES);
    $minAge = htmlspecialchars($_GET['minAge'], ENT_QUOTES);
    $maxAge = htmlspecialchars($_GET['maxAge'], ENT_QUOTES);
    $size = htmlspecialchars($_GET['size'], ENT_QUOTES);
    $color = htmlspecialchars($_GET['color'], ENT_QUOTES);
    $healthcare = htmlspecialchars($_GET['healthcare'], ENT_QUOTES);
    $breed === 'Choose' ? $breed = '' : $breed;
    $gender === 'Choose' ? $gender = '' : $gender;
    $size === 'Choose' ? $size = '' : $size;
    $color === 'Choose' ? $color = '' : $color;
    $healthcare === 'Choose' ? $healthcare = '' : $healthcare;
    $sql_ps = "SELECT p.pet_id,b.name as bname, b.isFeline, gender, age,p.name,story,diet,u.zipcode,datediff(now(),a.advertised_date) as days FROM pet_details p
                    join ads a on p.pet_id=a.pet_id
                    join users u on p.owner_id =u.user_id
                    join breeds b on p.breed_id= b.breed_id
